...
主仓库 CI 需要读取私有 `Jnove/mentor-agent-kb`。仓库管理员应为知识库创建一把专用、
**只读** deploy key，并把对应私钥保存为主仓库 Actions secret `KB_DEPLOY_KEY`。不得复用
个人 SSH 私钥或长期 PAT；密钥只应能读取该知识库仓库，并纳入定期轮换。
