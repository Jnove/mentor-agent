# -*- coding: utf-8 -*-
"""把 soaa 审核 keep 的篇目从 data/kb_staging 发布到 knowledge_base。
...
目标 = staging 相对路径去掉首个 soaa-<来源>/ 段。
用法：python scripts/soaa_promote.py [--dry-run]
"""
import io
import json
import os
...
FILES = ["soaa_agentA_keep.txt", "soaa_agentB_keep.txt", "soaa_agentC_keep.txt"]
OUT_JSON = os.path.join(STAGING, "_promote_soaa_final.json")
...
def kb_target(staging_rel):
    ...
    # src 以 soaa-<来源>/ 开头；重分类 dst 可能只写 KB 相对路径（如 政策/教育学院/招生）
    if parts[0].startswith("soaa-"):
        parts = parts[1:]
