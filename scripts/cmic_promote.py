# -*- coding: utf-8 -*-
"""把 cmic 审核 keep 的篇目从 data/kb_staging 发布到 knowledge_base。
...
目标 = staging 相对路径去掉首个 cmic-<来源>/ 段。
用法：python scripts/cmic_promote.py [--dry-run]
"""
import io
import json
import os
...
FILES = ["cmic_agentA_keep.txt", "cmic_agentB_keep.txt", "cmic_agentC_keep.txt", "cmic_agentD_keep.txt", "cmic_agentE_keep.txt", "cmic_agentF_keep.txt", "cmic_agentG_keep.txt"]
OUT_JSON = os.path.join(STAGING, "_promote_cmic_final.json")
...
def kb_target(staging_rel):
    ...
    # src 以 cmic-<来源>/ 开头；重分类 dst 可能只写 KB 相对路径（如 政策/传媒学院/招生）
    if parts[0].startswith("cmic-"):
        parts = parts[1:]
