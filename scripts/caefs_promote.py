# -*- coding: utf-8 -*-
"""把 caefs 审核 keep 的篇目从 data/kb_staging 发布到 knowledge_base。
...
目标 = staging 相对路径去掉首个 caefs-<来源>/ 段。
用法：python scripts/caefs_promote.py [--dry-run]
"""
import io
import json
import os
...
FILES = ["caefs_agentA_keep.txt", "caefs_agentB_keep.txt"]
OUT_JSON = os.path.join(STAGING, "_promote_caefs_final.json")
...
def kb_target(staging_rel):
    ...
    # src 以 caefs-<来源>/ 开头；重分类 dst 可能只写 KB 相对路径（如 政策/教育学院/招生）
    if parts[0].startswith("caefs-"):
        parts = parts[1:]
