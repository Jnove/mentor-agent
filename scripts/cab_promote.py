# -*- coding: utf-8 -*-
"""把 cab 审核 keep 的篇目从 data/kb_staging 发布到 knowledge_base。
...
目标 = staging 相对路径去掉首个 cab-<来源>/ 段。
用法：python scripts/cab_promote.py [--dry-run]
"""
import io
import json
import os
...
FILES = ["cab_agentA_keep.txt", "cab_agentB_keep.txt", "cab_agentC_keep.txt", "cab_agentD_keep.txt", "cab_agentE_keep.txt"]
OUT_JSON = os.path.join(STAGING, "_promote_cab_final.json")
...
def kb_target(staging_rel):
    ...
    # src 以 cab-<来源>/ 开头；重分类 dst 可能只写 KB 相对路径（如 政策/教育学院/招生）
    if parts[0].startswith("cab-"):
        parts = parts[1:]
