# -*- coding: utf-8 -*-
"""把 ghls 审核 keep 的篇目从 data/kb_staging 发布到 knowledge_base。
...
目标 = staging 相对路径去掉首个 ghls-<来源>/ 段。
用法：python scripts/ghls_promote.py [--dry-run]
"""
import io
import json
import os
...
FILES = [f"ghls_agent{c}_keep.txt" for c in "ABCD"]
OUT_JSON = os.path.join(STAGING, "_promote_ghls_final.json")
...
def kb_target(staging_rel):
    ...
    # src 以 ghls-<来源>/ 开头；重分类 dst 可能只写 KB 相对路径（如 政策/光华法学院/招生）
    if parts[0].startswith("ghls-"):
        parts = parts[1:]
    ...
    for fname in FILES:
        for line in read_keep(fname):
            ...
            if is_fix and not dst_rel.lower().endswith(".md"):
                # 重分类目标可只写目录（如 政策/光华法学院/招生），自动补源文件名
